...
CREATE TABLE IF NOT EXISTS `fog`.`user_orders` (
  ...
  INDEX `fk_user_orders_users2_idx` (`admin_id` ASC) VISIBLE,
  CONSTRAINT `fk_user_orders_users1`
    FOREIGN KEY (`user_id`)
    REFERENCES `fog`.`users` (`id_user`),
  CONSTRAINT `fk_user_orders_users2`
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `fog`.`billofmaterial` (`id_bom` INT, `order_id` INT, `quantity` INT, `description` INT, `id_material` INT, `name` INT, `type` INT, `cost` INT, `price` INT, `length` INT, `height` INT, `width` INT, `unit` INT);
