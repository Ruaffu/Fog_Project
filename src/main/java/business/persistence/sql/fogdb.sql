...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `fog`.`billofmaterial` (`id_bom` INT, `order_id` INT, `quantity` INT, `type` INT, `description` INT, `id_material` INT, `name` INT, `cost` INT, `price` INT, `length` INT, `height` INT, `width` INT, `unit` INT);

-- -----------------------------------------------------
-- Placeholder table for view `fog`.`userinfo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `fog`.`userinfo` (`id_user` INT, `email` INT, `password` INT, `role` INT, `firstname` INT, `lastname` INT, `streetname` INT, `housenr` INT, `zipcode` INT, `phonenr` INT, `city` INT, `region_name` INT);
...
USE `fog`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `fog`.`billofmaterial` AS select `fog`.`bom`.`id_bom` AS `id_bom`,`fog`.`bom`.`order_id` AS `order_id`,`fog`.`bom`.`quantity` AS `quantity`,`fog`.`bom`.`type` AS `type`,`fog`.`bom`.`description` AS `description`,`m`.`id_material` AS `id_material`,`m`.`name` AS `name`,`m`.`cost` AS `cost`,`m`.`price` AS `price`,`m`.`length` AS `length`,`m`.`height` AS `height`,`m`.`width` AS `width`,`m`.`unit` AS `unit` from (`fog`.`bom` join `fog`.`material` `m` on((`m`.`id_material` = `fog`.`bom`.`material_id`)));
...
USE `fog`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `fog`.`userinfo` AS select `u`.`id_user` AS `id_user`,`u`.`email` AS `email`,`u`.`password` AS `password`,`u`.`role` AS `role`,`u`.`firstname` AS `firstname`,`u`.`lastname` AS `lastname`,`u`.`streetname` AS `streetname`,`u`.`housenr` AS `housenr`,`u`.`zipcode` AS `zipcode`,`u`.`phonenr` AS `phonenr`,`c`.`city` AS `city`,`c`.`region_name` AS `region_name` from (`fog`.`users` `u` join `fog`.`city` `c` on((`u`.`zipcode` = `c`.`zip_code`)));
